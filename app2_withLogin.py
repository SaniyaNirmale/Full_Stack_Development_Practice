...
def home():
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS records2 (id INTEGER PRIMARY KEY AUTOINCREMENT ,user_id INTEGER, A REAL , B REAL , operations TEXT , result REAL)")
    cursor.execute("CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY AUTOINCREMENT,username TEXT UNIQUE,password TEXT)")

    if result is not None:
        user_id = session["user_id"]

        cursor.execute("INSERT INTO records2 (user_id,A,B,operations , result) VALUES(?,?,?,?,?)",(user_id, A , B , operations , result))
...
def history():
    ...
    cursor.execute("SELECT * FROM records2 WHERE user_id=?",(user_id,))
    records2 = cursor.fetchall()
    conn.close()
...
def delete(id):
    ...
    cursor.execute("DELETE FROM records2 WHERE id=? AND user_id=?",(id,session["user_id"]))
...
def edit(id):
    ...
    cursor.execute("SELECT * FROM records2 WHERE id=? AND user_id=?",(id,session["user_id"]))
    records1 = cursor.fetchone()
...
def update(id):
    ...
    cursor.execute("UPDATE records2 SET A = ? , B = ? , operations = ? , result=? WHERE id=? AND user_id=?",(A , B ,operations,result,id,session["user_id"]))
